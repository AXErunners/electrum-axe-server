IRC is used by Electrum AXE server to find 'peers' - other Electrum AXE servers. The
current list can be seen by running:

  electrum-axe-server peers

The following config file options are used by the IRC part of Electrum AXE server:
...
the Electrum AXE server will run in private mode (default). In private
mode, `electrum-axe-server peers` will always return an empty list.

`host` is a fully-qualified domain name (FQDN) of your Electrum AXE
...
your Electrum AXE server instead of `host`. It is used as part of the name
...
composing the IRC nickname to identify your server on #electrum-axe .
